...
The default explicit-deletion shape is object-level `state: absent`. CI and the
Dispatcher both reject that declaration unless the effective resource metadata
sets `deletion_supported: true` and records non-empty `deletion_evidence`.
...
| `aap_user_accounts` | false | Normal platform desired state; not generated by Bootstrap |
| `gateway_role_definitions` | false | Not audited for safe delete |
| `gateway_role_user_assignments` | false | Normal platform desired state; not generated by Bootstrap |
| `gateway_role_team_assignments` | false | Normal platform desired state; not generated by Bootstrap |
